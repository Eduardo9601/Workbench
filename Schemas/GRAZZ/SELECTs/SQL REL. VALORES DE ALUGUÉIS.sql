          ...
          SELECT MAX(SUB.ANO_REFERENCIA * 100 + SUB.MES_REFERENCIA)
           ...
           WHERE SUB.COD_UNIDADE = A.COD_UNIDADE
             ...
             AND (SUB.ANO_REFERENCIA * 100 + SUB.MES_REFERENCIA) <= 
                 TO_NUMBER(TO_CHAR(TO_DATE(:PERIODO, 'MM/YYYY'), 'YYYYMM'))
         )
...
/*== VERSÃO PARA EXECUTAR MANUALMENTE ==*/
...
BASE_CONTRATOS AS (
  ...
  SELECT A.COD_UNIDADE,
         ...
         B.DTA_DESCTO_INI,
         B.DTA_DESCTO_FIM
    FROM GRAZZ.GRZ_LOC_CONTRATOS@NLGRZ A
    JOIN GRAZZ.GRZ_LOC_CONTR_VALORES@NLGRZ B ON B.COD_CONTRATO = A.COD_CONTRATO
    LEFT JOIN SISLOGWEB.V_UNIDADES_ATIVAS@NLGRZ V ON V.COD_UNIDADE = A.COD_UNIDADE
   WHERE A.COD_EMPRESA = 1
     AND A.IND_INATIVO = 0
     AND B.IND_INATIVO = 0
     AND B.COD_VALOR = 1
...
LOJAS_EMPRESTIMOS AS (
  SELECT A.COD_UNIDADE,
         ...
         SUM(NVL(A.VLR_ADIANTAMENTO, 0)) AS VLR_EMPRESTIMO,
         NULL AS EMPRESTIMO_ATE
    FROM GRAZZ.GRZ_LOC_MVTO@NLGRZ A
    JOIN GRAZZ.GRZ_LOC_ADIANTAMENTOS@NLGRZ B ON B.COD_PESSOA = A.COD_PESSOA
                                      AND B.COD_CONTRATO = A.COD_CONTRATO
   /* BUSCA O ÚLTIMO MÊS DISPONÍVEL ATÉ O PERÍODO INFORMADO */
   WHERE (A.ANO_REFERENCIA * 100 + A.MES_REFERENCIA) = (
          SELECT MAX(SUB.ANO_REFERENCIA * 100 + SUB.MES_REFERENCIA)
            FROM GRAZZ.GRZ_LOC_MVTO@NLGRZ SUB
           WHERE SUB.COD_UNIDADE = A.COD_UNIDADE
             AND SUB.COD_CONTRATO = A.COD_CONTRATO
             AND SUB.VLR_ADIANTAMENTO <> 0
             AND (SUB.ANO_REFERENCIA * 100 + SUB.MES_REFERENCIA) <=

/* ========================================================== 
   ========= 2º VERSÃO SEM ENTRADA DE DATA NA EXECUÇÃO ========= 
   ========================================================== */
...
IS
  v_dta_ref DATE := CASE
                      WHEN p_dta_ref IS NOT NULL THEN TRUNC(p_dta_ref, 'MM')
...
BEGIN
  ...
  ------------------------------------------------------------------------------
  -- Remove carga anterior do mesmo mês
  ------------------------------------------------------------------------------
  ...
  AGREGADO AS (
    SELECT A.COD_EMP,
      ...
      JOIN NL.PS_PESSOAS P1
        ON P1.COD_PESSOA = A.COD_UNIDADE
     CROSS JOIN PARAMS PRM
     WHERE A.DTA_MVTO >= ADD_MONTHS(PRM.DTA_REF, -11)
       AND A.DTA_MVTO <  ADD_MONTHS(PRM.DTA_REF, 1)
         ...
         ROUND(
           360 / CASE
                   WHEN (SUM(A.VLR_CUSTO) /
                         CASE
                           WHEN (SUM(A.VLR_ESTMEDIO_PERM) / 365.25) > 0
...
/* ========================================================== 
   ========= 1º VERSÃO COM ENTRADA DE DATA NA EXECUÇÃO ========= 
   ========================================================== */

/*SCRIPT USADO SOMENTE PARA DELETAR REGISTROS NOS MOVIMENTOS DO CONTRATO, 
PARA O VD 905 ANTECIPAÇÃO, 
PARA COLABORADORES DEMITIDOS

OBS: PROCESSO REALIZADO APENAS EM CASO DE TER VALOR NO VD 905 PARA DEMITIDOS*/
...
WHERE EXISTS (
  ...
  WHERE A.COD_CONTRATO = B.COD_CONTRATO
    ...
    AND A.COD_VD = 905
    AND A.DATA_MOV = TO_DATE('29/02/2024', 'DD/MM/YYYY') -- Garantindo o uso correto de datas
    AND A.COD_EVENTO = 1
    AND A.PRESTACAO_VD = 99
    AND A.VALOR_VD <> 0
    AND A.COD_ORIGEM_MOV <> 1
...
/*SCRIPT USADO PARA INSERIR OS DADOS NOS MOVIMENTOS DO CONTRATO, 
AO QUAL FOI DELETADO POIS TINHA INSERIDO DUPLICADO OS VALORES NO AUMENTO DE 2024*/
...
AI COM ISSO FEITO, REPASSA NOVAMENTE AO RH A CONFIRMAÇÃO DA EXECUÇÃO, PARA QUE ENTÃO, O RH POSA GERAR O EVENTO 1 - MENSAL,
...
AND A.PRESTACAO_VD = 99
AND A.COD_EVENTO = 1
AND A.DATA_MOV = TO_DATE('29/02/2024', 'DD/MM/YYYY')
AND B.DATA_FIM IS NULL
...
INSERT INTO RHFP1004
  (SELECT 1 AS COD_EVENTO,
          ...
          CC.DATA_REFERENCIA,
          'Aumento Salarial (Evento 22) - Mestre 16619' AS OBSERVACAO,
          NULL
     FROM RHFP1005 CC
    WHERE CC.COD_EVENTO = 22
      ...
      AND CC.COD_CONTRATO IN (SELECT ORG.COD_CONTRATO
                               ...
                               WHERE CC.COD_CONTRATO = ORG.COD_CONTRATO
                                    --AND ORG.COD_CONTRATO = 389622 /*FILTRO PARA FINS DE TESTES, ANTES DA GERAÇÃO GERAL*/
                                 AND ORG.DATA_FIM = '31/12/2999'
                                 AND ORG.COD_NIVEL2 = 4) /*INSERIR SOMENTE A EMPRESA 8 (DEMAIS EMPRESAS: 4, 276, 1629)*/
   );
 ...
 (SELECT 1
   ...
   AND MOV.COD_CONTRATO IN (SELECT ORG.COD_CONTRATO
                             ...
                             WHERE MOV.COD_CONTRATO = ORG.COD_CONTRATO
                                  --AND ORG.COD_CONTRATO = 394553  /*FILTRO PARA FINS DE TESTES, ANTES DA GERAÇÃO GERAL*/
                               AND ORG.DATA_FIM = '31/12/2999'
                               AND ORG.COD_NIVEL2 = 276) /*INSERIR SOMENTE A EMPRESA 8 (DEMAIS EMPRESAS: 4, 276, 1629)*/

/*VERSÃO DO SQL PARA GERAR DADOS DA PLR - SOLICITAÇÃO DA RENATA*/
...
WITH
  Base_V AS (
    ...
    WHERE
      ...
      AND A.VALOR_VD <> 0
      --AND E.EDICAO_ORG IN ('705','707')
      AND (G.COD_REDE IN (70)
      OR E.EDICAO_ORG IN ('720','721', '722', '723', '764', '783', '786', '787', '907') )
    GROUP BY
      G.COD_REDE,
      G.DES_REDE,
      A.COD_VD,
  ...
  Base_D AS (
    SELECT
      ...
      END AS NOME_VD,
      SUM(NVL(A.VALOR_VD, 0)) AS TOTAL_MES,
      ROW_NUMBER() OVER (PARTITION BY G.COD_REDE ORDER BY CASE WHEN A.COD_VD = 1003 THEN 1004 ELSE A.COD_VD END) AS RN
    FROM
      RHFP1006 A
      INNER JOIN RHFP1000 B ON A.COD_VD = B.COD_VD
...
END) AS TOTAL_LIQUIDO
  ...
  Combined_Base AS (
    SELECT DISTINCT
      ...
      NULL AS NOME_VD_V,
      /*TO_CHAR(V.TOTAL_MES, 'FM999G999G990D00')*/ NULL AS TOTAL_MES_V,
      /*CASE
      ...
      NULL AS NOME_VD_D,
      /*TO_CHAR(D.TOTAL_MES, 'FM999G999G990D00')*/ NULL AS TOTAL_MES_D,
      /*CASE
      ...
      NULL AS NOME_VD_O,
      /*TO_CHAR(O.TOTAL_MES, 'FM999G999G990D00')*/ NULL AS TOTAL_MES_O,

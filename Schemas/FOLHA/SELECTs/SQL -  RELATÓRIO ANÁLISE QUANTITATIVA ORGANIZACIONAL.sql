...
NIVEIS AS (
  ...
  SELECT 3,        'DIR_ESTAT'      ,          'DIRETORIA ESTATUTÁRIA'   FROM DUAL UNION ALL
  SELECT 4,        'DIRETORIA'      ,          'DIRETORIA'               FROM DUAL UNION ALL
  SELECT 5,        'GER_SUP'        ,          'GERÊNCIA/SUPERVISÃO'     FROM DUAL UNION ALL
  SELECT 6,        'ESPECIAL'       ,          'ESPECIALISTAS'           FROM DUAL UNION ALL
  SELECT 7,        'ADMIN'          ,          'ADMINISTRATIVO'          FROM DUAL UNION ALL
  SELECT 8,        'OPER'           ,          'OPERACIONAL'             FROM DUAL UNION ALL
  SELECT 9,        'APRENDIZ'       ,          'APRENDIZ'                FROM DUAL UNION ALL
  SELECT 10,       'ESTAGIARIO'     ,          'ESTAGIÁRIO'              FROM DUAL
),
...
CLASSIF AS (
  SELECT
    ...
    CASE
      ...
      WHEN COD_CONTRATO IN (299685, 375780, 386492) THEN 'DIR_ESTAT'

      /* 4) Diretoria */
      WHEN DES_NORM LIKE 'DIRETOR%' AND COD_CONTRATO NOT IN (299685, 375780, 386492) THEN 'DIRETORIA'

      /* 5) Gerência/Supervisão */
      ...
      WHEN (DES_NORM LIKE 'ANALISTA%' OR DES_NORM LIKE 'PROGRAMADOR%' OR
            ...
            DES_NORM LIKE 'INSTRUTOR%' OR DES_NORM LIKE 'TECNIC%' OR
      ...
      /* 7) Administrativo (override assessor) */
      ...
      WHEN (DES_NORM LIKE 'AUXILIAR DE ESCRITORIO%' OR DES_NORM LIKE 'AUXILIAR DE PESSOAL%' OR
            ...
            DES_NORM LIKE 'ASSISTENTE ADMINISTRATIVO%' OR DES_NORM LIKE 'ASSISTENTE CONTABIL%' OR
            DES_NORM LIKE 'ASSISTENTE FISCAL%' OR DES_NORM LIKE 'ASSISTENTE FINANCEIRO%') THEN 'ADMIN'

      /* 8) Operacional */
      WHEN (DES_NORM LIKE '%LOJA%' OR DES_NORM LIKE 'VENDEDOR%' OR DES_NORM LIKE 'VEND.%' OR
            ...
            DES_NORM LIKE '%OPERADOR%' OR DES_NORM LIKE '%ESTOQUE%' OR DES_NORM LIKE '%REPOSITOR%' OR
            ...
            DES_NORM LIKE '%ORIENTADOR DE VENDA%' OR DES_NORM LIKE '%EXTRACAO%') THEN 'OPER'
...
BLOCO2_BASE AS (
  SELECT * FROM GRID_I
   WHERE NIVEL_KEY IN ('CONSELHO_ADM','CONSELHO_FISC','DIR_ESTAT')
),
...
BLOCO2_LINHAS AS (
  ...
  SELECT PERIODO, ANO, 3, NIVEL_DESC, FEM_QTDE, FEM_PCT, MASC_QTDE, MASC_PCT, TOTAL
    FROM BLOCO2_BASE WHERE NIVEL_KEY='DIR_ESTAT'
),

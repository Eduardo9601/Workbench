  ...
  DECLARE
    ...
    CURSOR c_orcamento is
          select a.cod_unidade
                ,to_char(a.dta_orcamento,'mm/yyyy') mes_ano
                ,substr(a.des_chave,1,2) des_chave
                ,round(sum(nvl(a.vlr_realizado,0)),2) vlr_realizado
            from or_valores a
           where a.cod_emp = pi_emp
             and a.cod_unidade = r_venda_bruta.cod_unidade
             and a.cod_orcamento = 400
             and a.dta_orcamento >= pi_data_ini
             and a.dta_orcamento <= pi_data_fim
             and (a.des_chave = '01#01001'  -- venda bruta
               or a.des_chave = '01#01003'
               or a.des_chave = '01#01005'
               or a.des_chave = '05#05001'  -- venda liquida
               or a.des_chave = '15#15001'  -- despesas com pessoal
               ...
               or a.des_chave = '45#45001') -- lucro
           group by a.cod_unidade
                   ,to_char(a.dta_orcamento,'mm/yyyy')
                   ,substr(a.des_chave,1,2)
           order by 1,2,3;

       r_orcamento c_orcamento%ROWTYPE;
  ...
  BEGIN
    ...
    WHILE c_venda_bruta%FOUND LOOP
      BEGIN
        ...
        begin
          select count(1) total_colaboradores
           ...
           where edicao_filial = wEdicaoPdLaft
             and data_fim is null
             and cod_clh not in (71, 74, 68) -- aprendiz e servente n?o entram
             and status_afastamento not in (0, 7, 107) -- 0 padrao / 7 ferias / 107 afastamento temporario
             and (instr(',' || wEdicao || ',', ',' || edicao_nivel3 || ',') > 0);
        exception
          when no_data_found then
            wQtdAfastados := 0;
        ...
        begin
           SELECT SUM(NVL(VLR_HORAS_EXTRAS, 0)) VLR_HORAS_EXTRAS,
                  SUM(NVL(VLR_BONUS_DOM, 0)) VLR_BONUS_DOM

...
BEGIN
  DECLARE
    ...
    /**** sql alternativo para venda bruta ****/
      select ge.cod_quebra regiao,
             ...
             SUM(CASE
                 ...
                 END) vda_domingo
        from NL.ns_notas a
        join NL.ns_notas_operacoes oper on a.num_seq = oper.num_seq
                                       and a.cod_maquina = oper.cod_maquina
  ...
  BEGIN
    ...
    WHILE c_venda_bruta%FOUND LOOP
      BEGIN
        ...
        /**** bloco if LOJAS MIGRADAS - tratar lojas que sofreram migração de rede ****/
        if r_venda_bruta.cod_unidade = 22 then
          wLoja := 7022;
        elsif r_venda_bruta.cod_unidade = 47 then
          wLoja := 7047;
        elsif r_venda_bruta.cod_unidade = 652 then
          wLoja := 7051;
        elsif r_venda_bruta.cod_unidade = 65 then
          wLoja := 7065;
        elsif r_venda_bruta.cod_unidade = 654 then
          wLoja := 7066;
        elsif r_venda_bruta.cod_unidade = 138 then
          wLoja := 7138;
        elsif r_venda_bruta.cod_unidade = 140 then
          wLoja := 7140;
        elsif r_venda_bruta.cod_unidade = 183 then
          wLoja := 7183;
        elsif r_venda_bruta.cod_unidade = 244 then
          wLoja := 7244;
        elsif r_venda_bruta.cod_unidade = 353 then
          wLoja := 7353;
        elsif r_venda_bruta.cod_unidade = 386 then
          wLoja := 7412;
        elsif r_venda_bruta.cod_unidade = 430 then
          wLoja := 7430;
        elsif r_venda_bruta.cod_unidade = 442 then
          wLoja := 7442;
        elsif r_venda_bruta.cod_unidade = 461 then
          wLoja := 7461;
        elsif r_venda_bruta.cod_unidade = 491 then
          wLoja := 7491;
        elsif r_venda_bruta.cod_unidade = 651 then
          wLoja := 7500;
        elsif r_venda_bruta.cod_unidade = 543 then
          wLoja := 7543;
        elsif r_venda_bruta.cod_unidade = 555 then
          wLoja := 7555;
        elsif r_venda_bruta.cod_unidade = 570 then
          wLoja := 7570;
        elsif r_venda_bruta.cod_unidade = 653 then
          wLoja := 7577;
        elsif r_venda_bruta.cod_unidade = 587 then
          wLoja := 7587;
        elsif r_venda_bruta.cod_unidade = 588 then
          wLoja := 7588;
        elsif r_venda_bruta.cod_unidade = 592 then
          wLoja := 7592;
        elsif r_venda_bruta.cod_unidade = 597 then
          wLoja := 7597;
        elsif r_venda_bruta.cod_unidade = 601 then
          wLoja := 7601;
        elsif r_venda_bruta.cod_unidade = 602 then
          wLoja := 7602;
        elsif r_venda_bruta.cod_unidade = 620 then
          wLoja := 7620;
        else
          wLoja := r_venda_bruta.cod_unidade;
        end if;
        ...
        begin
          select round(sum(nvl(a.vlr_realizado, 0)), 2) vlr_realizado,
            ...
            Into wVlrLiqOrc, wDta_Orcamento
            from or_valores a
           where a.cod_emp = pi_emp
             and a.cod_unidade in (wLoja, r_venda_bruta.cod_unidade)
        ...
        if (wDta_Orcamento < '01/' || substr(pi_data_fim, 4, 7)) then
            ...
            select round(sum(nvl(a.vlr_realizado, 0)), 2) vlr_realizado
             ...
             where a.cod_emp = pi_emp
               ...
               and (a.des_chave = '01#01001' -- venda bruta
                   or a.des_chave = '01#01003' or a.des_chave = '01#01005');
          exception
            when no_data_found then
              wVlrVdaBrutaOrc := 0;
          end;
          ...
          begin
            select sum(decode(a.tip_lancamento,
                              2,
                              nvl(a.vlr_medio_emp, 0),
                              1,
                              (nvl(a.vlr_medio_emp, 0) * -1),
                              0)) vlr_custo,
                   sum(decode(a.tip_lancamento,
                              2,
                              nvl(a.vlr_icms, 0),
                              1,
                              (nvl(a.vlr_icms, 0) * -1),
                   ...
                   sum(decode(a.tip_lancamento,
                              ...
                              nvl(d.vlr_cofins, 0),
                              1,
                              (nvl(a.vlr_cofins, 0) * -1),
                              0)) vlr_cofins,
                   sum(decode(a.tip_lancamento, 1, nvl(a.vlr_total, 0), 0)) devolucao
              INTO wVlrCusto, wVlrICMS, wVlrPIS, wVlrCONFINS, wVlrDevolucao
              from nl.ce_diarios a, nl.t6_oper_quebras o, nl.ns_itens d
          ...
          if r_venda_bruta.valor_venda_loja > 0 then
            wVdaLiquida := (r_venda_bruta.valor_venda_loja -
                           nvl(wVlrVdaBrutaOrc, 0)) - nvl(wVlrCusto, 0) -
                           nvl(wVlrICMS, 0) - nvl(wVlrPIS, 0) -
                           nvl(wVlrConfins, 0) - nvl(wVlrDevolucao, 0);
          else
            wVdaLiquida := 0;
          end if;
        end if;
        ...
        begin
          select sum(oper.vlr_operacao) valor_venda_loja
           ...
           where a.num_seq = oper.num_seq
             ...
             and a.ind_status = 1
             and a.tip_nota = 4
             and a.dta_emissao between add_months(pi_data_ini, -12) and
                 add_months(pi_data_fim, -12);
        exception
          when no_data_found then
        ...
        if nvl(wVlrVendaBrutaAnoAnt, 0) <= 0 then
          ...
          wPerCresc := ((r_venda_bruta.valor_venda_loja * 100) /
                       nvl(wVlrVendaBrutaAnoAnt, 0)) - 100;
        end if;
        ...
        begin
          select substr(hora_ent, 1, 2) || ':' || substr(hora_ent, 3, 2) entrada,
            ...
            Into wHorAbertura, wHorFechamento
          ...
          select round(sum(nvl(a.vlr_realizado, 0)), 2) vlr_realizado
           ...
           where a.cod_emp = pi_emp
             ...
             and (a.des_chave = '15#15001' -- despesas com pessoal
                 or a.des_chave = '15#15005' or a.des_chave = '15#15010' or
                 a.des_chave = '15#15015' or a.des_chave = '15#15020' or
                 a.des_chave = '15#15025' -- despesas com vale-transporte
                 or a.des_chave = '15#15030' -- despesas com vale-transporte
                 or a.des_chave = '15#15035' or a.des_chave = '15#15040' or
        ...
        begin
          SELECT COALESCE(SUM(VLR_HORAS_EXTRAS), 0) AS VLR_HORAS_EXTRAS,
            ...
            INTO WVLR_HORAS_EXTRAS, WVLR_BONUS_DOM
            FROM GRZ_FOLHA.V_VALOR_HORAS_EXTRAS_AVT@GRZFOLHA
           WHERE COD_UNIDADE in (wLoja, r_venda_bruta.cod_unidade)

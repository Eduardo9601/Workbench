...
NIVEIS AS (
  SELECT 1 ORDEM,  'DIRETORIA_ESTAT'   NIVEL_KEY, 'DIRETORIA ESTATUTÁRIA'  NIVEL_DESC FROM DUAL UNION ALL
  SELECT 2,        'DIRETORIA_ADJ',             'DIRETORIA ADJUNTA'                  FROM DUAL UNION ALL
  SELECT 3,        'GER_SUP',                   'GERÊNCIA/SUPERVISÃO'                FROM DUAL UNION ALL
  SELECT 4,        'ESPECIAL',                  'ESPECIALISTAS'                      FROM DUAL UNION ALL
  SELECT 5,        'ADMIN',                     'ADMINISTRATIVO'                     FROM DUAL UNION ALL
  SELECT 6,        'OPER',                      'OPERACIONAL'                        FROM DUAL UNION ALL
  SELECT 7,        'APRENDIZ',                  'APRENDIZ'                           FROM DUAL UNION ALL
  SELECT 8,        'ESTAGIARIO',                'ESTAGIÁRIO'                         FROM DUAL
),
...
CLASSIF AS (
  SELECT
    ...
    CASE
     ...
     /* 2) Diretoria estatutária*/
      WHEN COD_CONTRATO IN (376962, 373907, 379872) THEN 'DIRETORIA_ESTAT'

      /* 3) Diretoria */
      WHEN DES_NORM LIKE 'DIRETOR%' AND COD_CONTRATO NOT IN (376962, 373907, 379872) THEN 'DIRETORIA_ADJ'

      /* 4) Gerência/Supervisão */
      ...
      /* 6) Administrativo (override assessor) */
      ...
      WHEN (DES_NORM LIKE '%ADMINISTRAT%' OR DES_NORM LIKE '%ESCRITORIO%' OR DES_NORM LIKE '%PESSOAL%' OR
            ...
            DES_NORM LIKE '%E-COMMERCE%' OR DES_NORM LIKE '%COMUNICACAO%' OR DES_NORM LIKE '%DESIGNER%' OR
      ...
      /* 7) Operacional */
      WHEN (DES_NORM LIKE '%AUXILIAR%' OR DES_NORM LIKE '%AUX.%' OR DES_NORM LIKE '%LOJA%' OR DES_NORM LIKE 'VENDEDOR%' OR DES_NORM LIKE 'VEND.%' OR
...
TOTAIS_I AS (
  ...
  FROM GRID_I
  --WHERE NIVEL_KEY <> 'DIR_ESTAT'   -- <<<<<<<<<< AQUI (não soma financeira no total geral)
...
BLOCO2_BASE AS (
  SELECT * FROM GRID_I
   WHERE NIVEL_KEY IN ('DIRETORIA_ESTAT')
),
...
BLOCO2_LINHAS AS (
  SELECT PERIODO, ANO, 1 AS B2_TARGET_ORDEM,
         ...
         TOTAL AS B2_TOTAL
    FROM BLOCO2_BASE WHERE NIVEL_KEY='DIRETORIA_ESTAT'
),
...
SELECT
  '4' AS "Cód. Empresa",
  '004 - Mundiart S/A' as "Empresa Relacionada",

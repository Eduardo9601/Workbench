...
/* =========================================================
   ...
   OBS: sem STATUS = 0 para não matar histórico válido
   ========================================================= */
CONTRATO_MES AS (
    SELECT
        X.COD_CONTRATO,
...
/* =========================================================
   ...
   LEFT depois para não sumir colaborador
   ========================================================= */
FUNCAO_MES AS (
    SELECT
        T.COD_CONTRATO,
...
/* =========================================================
   7) AFASTAMENTOS DO MÊS
   Mantida lógica de férias no bloco próprio.
   Aqui NÃO vou excluir todo mundo por afastamento genérico,
   porque teu pedido agora é retornar todos do período.
   ========================================================= */
FERIAS_MES AS (
    SELECT
        C.COD_CONTRATO,
...
/* =========================================================
   8) DIMENSÃO FINAL
   BASE MENSAL ÚNICA POR CONTRATO + MÊS
   ========================================================= */
DIM AS (
    SELECT
        B.STATUS,
...
/* =========================================================
   12) BASE MENSAL
   OBS: unidade/rede/tipo agora são HISTÓRICOS DO MÊS
   ========================================================= */
BASE_MENSAL AS (
    SELECT
        D.COD_CONTRATO,
...
/* =========================================================
   13) TOTAL POR COLABORADOR NO PERÍODO
   ========================================================= */
PERIODO_COLAB AS (
    SELECT
        COD_CONTRATO,
...
/* =========================================================
   14) RESULTADO FINAL
   AGORA EXIBE A LOTAÇÃO HISTÓRICA DO MÊS
   ========================================================= */
SELECT
    D.COD_ORGANOGRAMA,
    D.COD_CONTRATO,

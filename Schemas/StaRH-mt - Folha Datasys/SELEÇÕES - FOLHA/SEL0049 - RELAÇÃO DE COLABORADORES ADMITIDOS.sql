...
SELECT DISTINCT
       ...
       CASE
         WHEN A.COD_SINDICATO IS NOT NULL THEN
          A.COD_SINDICATO || ' - ' || A.DES_SINDICATO
         ELSE
          'Não Contém Informação'
       END AS SINDICATO,
       A.IND_CONT_SINDICAL,
  ...
  CROSS JOIN PARAMS P
 WHERE A.DATA_ADMISSAO BETWEEN P.ADMISSAO_INICIO AND P.ADMISSAO_FIM
   AND A.COD_EMP = 8
   --AND A.STATUS = 0
...
SELECT DISTINCT
 ...
 WHERE A.COD_MOTIVO = B.COD_MOTIVO(+)
   ...
   AND P.ADMISSAO_FIM BETWEEN A.DATA_INICIO AND A.DATA_FIM
   --AND a.COD_CONTRATO = 397017
...
FROM RHFP0304 A,
     ...
     RHFP0116 D,
     PARAMS P
WHERE P.ADMISSAO_FIM BETWEEN A.DATA_INICIO AND A.DATA_FIM
  ...
  FROM RHFP1006 A,
       (SELECT B.COD_MESTRE_EVENTO,
         ...
         WHERE B.SITUACAO_PROCESSO = 'I'
           AND B.DATA_REFERENCIA BETWEEN P.ADMISSAO_INICIO AND
               P.ADMISSAO_FIM) MESTRE

...
SELECT GE.COD_QUEBRA REGIAO,
  ...
  JOIN NL.T6_OPERACOES@NLGRZ T6 ON OPER.COD_OPER = T6.COD_OPER
                               AND T6.COD_GRUPO_OPER = 1
...
SELECT
    X.COD_EMP,
    X.COD_UNIDADE,
    SUM(X.QTD_220)         AS QTD_220,
    SUM(X.QTD_195)         AS QTD_195,
    SUM(X.QTD_180)         AS QTD_180,
    SUM(X.QTD_165)         AS QTD_165,
    SUM(X.QTD_150)         AS QTD_150,
    SUM(X.QTD_120)         AS QTD_120,
    SUM(X.QTD_110)         AS QTD_110,
    SUM(X.QTD_100)         AS QTD_100,
    SUM(X.COLABS_PROD)     AS COLABS_PROD,
    SUM(X.QTD_PCDS)        AS QTD_PCDS,
    SUM(X.QTD_TEMPORARIOS) AS QTD_TEMPORARIOS
FROM (
    SELECT
        ORG.COD_EMP,
        ORG.COD_UNIDADE,
        ...
        MAX(DECODE(TO_NUMBER(HR.HR_BASE_MES), 220, 1, 0)) AS QTD_220,
        MAX(DECODE(TO_NUMBER(HR.HR_BASE_MES), 195, 1, 0)) AS QTD_195,
        MAX(DECODE(TO_NUMBER(HR.HR_BASE_MES), 180, 1, 0)) AS QTD_180,
        MAX(DECODE(TO_NUMBER(HR.HR_BASE_MES), 165, 1, 0)) AS QTD_165,
        MAX(DECODE(TO_NUMBER(HR.HR_BASE_MES), 150, 1, 0)) AS QTD_150,
        MAX(DECODE(TO_NUMBER(HR.HR_BASE_MES), 120, 1, 0)) AS QTD_120,
        MAX(DECODE(TO_NUMBER(HR.HR_BASE_MES), 110, 1, 0)) AS QTD_110,
        MAX(DECODE(TO_NUMBER(HR.HR_BASE_MES), 100, 1, 0)) AS QTD_100,

        MAX(CASE
              WHEN CT.IND_DEFICIENCIA = 'N' THEN 1
              WHEN CT.IND_DEFICIENCIA = 'S'
                   AND TO_NUMBER(HR.HR_BASE_MES) > 120 THEN 1
              ELSE 0
            END) AS COLABS_PROD,

        MAX(DECODE(CT.IND_DEFICIENCIA, 'S', 1, 0)) AS QTD_PCDS,

        MAX(CASE WHEN FN.COD_FUNCAO = 204 THEN 1 ELSE 0 END) AS QTD_TEMPORARIOS
    ...
    WHERE CT.COD_CONTRATO = ORG.COD_CONTRATO
      ...
      AND :DATA_FIM BETWEEN CT.DATA_ADMISSAO AND NVL(CT.DATA_DEMISSAO, DATE '9999-12-31')
    ...
    GROUP BY ORG.COD_UNIDADE, ORG.COD_EMP, CT.COD_CONTRATO
) X
GROUP BY X.COD_EMP, X.COD_UNIDADE
),
...
SELECT ORG.COD_EMP,
       ORG.COD_UNIDADE,
       COUNT(DISTINCT CT.COD_CONTRATO) AS QTD_SERVENTES
  FROM V_DADOS_CONTRATO_AVT CT
  JOIN (SELECT COD_CONTRATO
          FROM (SELECT FN.COD_CONTRATO,
                       ROW_NUMBER() OVER(PARTITION BY FN.COD_CONTRATO ORDER BY FN.DATA_INI_CLH DESC, NVL(FN.DATA_FIM_CLH, DATE '9999-12-31') DESC) RN
                  FROM VH_CARGO_CONTRATO_AVT FN
                 WHERE FN.COD_FUNCAO = 68
                   AND :DATA_FIM /*TO_DATE('31/01/2026', 'DD/MM/YYYY')*/ BETWEEN
                       FN.DATA_INI_CLH AND
                       NVL(FN.DATA_FIM_CLH, DATE '9999-12-31'))
         WHERE RN = 1) FN
    ON FN.COD_CONTRATO = CT.COD_CONTRATO
  JOIN (SELECT COD_CONTRATO, COD_EMP, COD_UNIDADE
          FROM (SELECT ORG.COD_CONTRATO,
                       ORG.COD_EMP,
                       ORG.COD_UNIDADE,
                       ROW_NUMBER() OVER(PARTITION BY ORG.COD_CONTRATO ORDER BY ORG.DATA_INI_ORG DESC, NVL(ORG.DATA_FIM_ORG, DATE '9999-12-31') DESC) RN
                  FROM VH_EST_ORG_CONTRATO_AVT ORG
                 WHERE ORG.COD_TIPO = 1
                   AND :DATA_FIM /*TO_DATE('31/01/2026', 'DD/MM/YYYY')*/ BETWEEN
                       ORG.DATA_INI_ORG AND
                       NVL(ORG.DATA_FIM_ORG, DATE '9999-12-31'))
         WHERE RN = 1) ORG
    ON ORG.COD_CONTRATO = CT.COD_CONTRATO
 WHERE :DATA_FIM /*TO_DATE('31/01/2026', 'DD/MM/YYYY')*/ BETWEEN CT.DATA_ADMISSAO AND
       NVL(CT.DATA_DEMISSAO, DATE '9999-12-31')
   AND ORG.COD_UNIDADE NOT IN (14, 50, 615, 7586, 7608)
 GROUP BY ORG.COD_UNIDADE, ORG.COD_EMP
...
CONTRATOS_EMP AS (
  SELECT
      Y.COD_EMP,
      SUM(Y.QTD_220)         AS QTD_220,
      SUM(Y.QTD_195)         AS QTD_195,
      SUM(Y.QTD_180)         AS QTD_180,
      SUM(Y.QTD_165)         AS QTD_165,
      SUM(Y.QTD_150)         AS QTD_150,
      SUM(Y.QTD_120)         AS QTD_120,
      SUM(Y.QTD_110)         AS QTD_110,
      SUM(Y.QTD_100)         AS QTD_100,

      SUM(CASE
            WHEN Y.IND_DEFICIENCIA = 'S' THEN 0
            ELSE Y.HR_BASE_MES
          END) AS TOTAL_HRS,

      SUM(Y.COLABS_PROD)     AS COLABS_PROD,
      SUM(Y.QTD_PCDS)        AS QTD_PCDS,
      SUM(Y.QTD_TEMPORARIOS) AS QTD_TEMPORARIOS
  FROM (
      SELECT
          ORG.COD_EMP,
          CT.COD_CONTRATO,
          ...
          MAX(TO_NUMBER(HR.HR_BASE_MES)) AS HR_BASE_MES,
      ...
      GROUP BY ORG.COD_EMP, CT.COD_CONTRATO, CT.IND_DEFICIENCIA
  ) Y
  GROUP BY Y.COD_EMP
),

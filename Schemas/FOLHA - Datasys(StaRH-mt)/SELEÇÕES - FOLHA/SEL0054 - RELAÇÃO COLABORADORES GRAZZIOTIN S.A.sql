...
DADOS_CONTRATOS_UNI AS (
  SELECT A.COD_CONTRATO,
    ...
    FROM CONTRATOS A
    LEFT JOIN STATUS_AFASTADOS_UNI S ON A.COD_CONTRATO = S.COD_CONTRATO
    LEFT JOIN CNPJ_LOJAS CP ON CP.COD_UNIDADE = A.COD_UNIDADE
...
DADOS_CONTRATOS_REDE AS (
  SELECT A.COD_CONTRATO,
         ...
         A.EDICAO AS ORG_HIERARQUIA
    FROM CONTRATOS A
    LEFT JOIN STATUS_AFASTADOS_REDE S ON A.COD_CONTRATO = S.COD_CONTRATO
...
SELECT A.COD_UNIDADE AS UNIDADE,
       ...
       COUNT(DISTINCT CASE
           WHEN B.STATUS_AFAST NOT IN (0, 6, 7) THEN A.COD_CONTRATO
  ...
  LEFT JOIN STATUS_AFASTADOS_UNI B ON A.COD_CONTRATO = B.COD_CONTRATO
...
SELECT A.COD_REDE AS REDE,
  ...
  LEFT JOIN STATUS_AFASTADOS_REDE B ON A.COD_CONTRATO = B.COD_CONTRATO
 GROUP BY A.COD_REDE

WITH
  Base_V AS (
      SELECT
      CASE
        WHEN E.EDICAO_ORG BETWEEN 8701 AND 8716 THEN '723'
        ELSE E.EDICAO_ORG
      END AS EDICAO_ORG_REP,
      CASE
        WHEN E.EDICAO_ORG BETWEEN 8701 AND 8716 THEN '723'
        ELSE E.EDICAO_ORG
      END AS EDICAO_ORG,
      ...
      ROW_NUMBER() OVER (PARTITION BY TO_NUMBER(CASE WHEN E.EDICAO_ORG BETWEEN 8701 AND 8716 THEN '723' ELSE E.EDICAO_ORG END) ORDER BY A.COD_VD) AS RN
    FROM
      RHFP1006 A
      ...
      LEFT JOIN V_ORGANOGRAMAS_AVT ORG ON E.COD_ORGANOGRAMA = ORG.COD_ORGANOGRAMA
    WHERE
      ...
      AND A.VALOR_VD <> 0
      AND (:REDE = '0' OR ORG.COD_REDE IN (SELECT * FROM TABLE(SPLIT_CONTRACTS(:REDE))))
      --AND E.EDICAO_ORG IN ('750', '740')
    GROUP BY
      CASE WHEN E.EDICAO_ORG BETWEEN 8701 AND 8716 THEN '723' ELSE E.EDICAO_ORG END,
      CASE
        WHEN E.EDICAO_ORG = '815' THEN 'COMPRAS GRAZZIOTIN'
        WHEN E.EDICAO_ORG = '835' THEN 'COMPRAS POR MENOS'
        WHEN E.EDICAO_ORG = '845' THEN 'COMPRAS FRANCO GIORGI'
  ...
  Base_D AS (
    SELECT
      ...
      END AS NOME_VD,
      SUM(NVL(A.VALOR_VD, 0)) AS TOTAL_MES,
      ROW_NUMBER() OVER (PARTITION BY TO_NUMBER(CASE WHEN E.EDICAO_ORG BETWEEN 8701 AND 8716 THEN '723' ELSE E.EDICAO_ORG END) ORDER BY CASE WHEN A.COD_VD = 1003 THEN 1004 ELSE A.COD_VD END) AS RN
    FROM
      RHFP1006 A
      INNER JOIN RHFP1000 B ON A.COD_VD = B.COD_VD
  ...
  Base_O AS (
    ...
    WHERE
      ...
      AND E.EDICAO_NIVEL2 = '008'
      AND A.COD_VD IN (12, 885, 908, 910, 912, 914, 915, 916, 924, 925, 926, 927, 1004, 1063, 1064, 1067, 1068, 1231, 1232,
                       1233, 1234, 1456, 1457, 1458, 1459, 1461, 1462, 1463, 1464, 1910, 1924, 1934, 1935, 1941, 2222, 2232,
                       2601, 2602, 2605, 2611, 2612, 2615, 2616, 2617, 2701, 2705, 2706, 2711, 2712, 2713, 2714, 2715, 2716, 2717)
      AND A.VALOR_VD <> 0
      AND(:REDE = '0' OR ORG.COD_REDE IN (SELECT * FROM TABLE(SPLIT_CONTRACTS(:REDE))))
      --AND E.EDICAO_ORG IN ('750', '740')
    GROUP BY

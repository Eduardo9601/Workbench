...
DECLARE
	...
	select cia.cod_unidade_de cod_unidade
               ...
               where a.cod_unidade    = cia.cod_unidade_para
                 and cia.cod_emp_de   = PI_CODREDE
				 ...
				 and a.dta_orcamento not in ('01/03/2021','01/04/2021','01/07/2021')
               ...
               group by cia.cod_unidade_de
			   order by cia.cod_unidade_de;
	...
	BEGIN
	 wLucroRede := 0;
     wVendaLiqRede := 0;
	 select sum(Vlr_Lucro),sum(Vlr_Venda_Liquida) INTO wLucroRede,wVendaLiqRede  from (select    cia.cod_unidade_de,
                    sum(decode(a.des_chave,'05#05001',nvl(a.vlr_realizado,0),0)) Vlr_Venda_Liquida
                    ,sum(decode(a.des_chave,'45#45001',nvl(a.vlr_realizado,0),0)) Vlr_Lucro
                from or_valores a
                    ,grz_lojas_unificadas_cia cia
                    ,ps_pessoas p                    
               where a.cod_unidade    = cia.cod_unidade_para
                 and p.cod_pessoa     = cia.cod_unidade_de
                 and p.ind_inativo    = 0
                 and a.cod_emp        = pi_codemp
                 and a.cod_orcamento  = PI_ORCAMENTO
                 and a.dta_orcamento >= '01/01/'||SUBSTR(PI_DATA_REF,7,4)
       ...
       --           having sum(decode(a.des_chave,'45#45001',nvl(a.vlr_realizado,0),0)) >0
		...
		BEGIN
			...
			select NVL(round(sum(nvl(a.vlr_realizado,0)),2),0) vlr_realizado
                   ...
                   and cia.cod_unidade_de = r_totais_orcamento.cod_unidade
			 ...
			 begin
                             select sum(nvl(o.vlr_operacao,0)) vlr_operacao
                              ...
                              where t.num_seq     = o.num_seq
								...
								and (t.dta_emissao not between '01/03/2021' and '30/04/2021'
                                and t.dta_emissao not between '01/07/2021' and '31/07/2021')
                                and t.cod_unidade = r_totais_orcamento.cod_unidade
                                and nvl(t.ind_status,0) <> 2
                                and o.cod_oper in (4106, 4107);
	                  EXCEPTION
			...
			if r_totais_orcamento.vlr_venda_liquida > 0 then
			   ...
			   wMargem_Percentual := 99;
		        ...
		        if PI_CODREDE = 70 then
                           if wMargem_Percentual >= 20.0 then

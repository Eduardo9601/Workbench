	...
	BEGIN
	 ...
	 select sum(Vlr_Lucro),sum(Vlr_Venda_Liquida) INTO wLucroRede,wVendaLiqRede  from (select    a.cod_unidade,               
		...
		BEGIN		
	       ...
	       begin
			...
			if (wHoras_Trab <> 0) then
			 ...
			 if (r_totais_orcamento.Vlr_Venda_Real >= wVendaOrcada ) then 
			    wPontosOrcado := 5;
		     else		
			    wPontosOrcado := 0;
			 end if;	
		 ...
		 */
             ...
             ELSE  
             BEGIN			 
                              insert into GRZ_DADOS_CALCULO_APR_ANUAL (COD_UNIDADE,ORCADO,MARGEM,LUCRO,INVENTARIO,VALE_TRANSP,PRODUTIVIDADE,FOLHA,COD_REDE,TIP_UNIDADE,REGIAO,ANO,DATA_REFERENCIA)
							  ...
							  PI_CODREDE,1,wRegiao,SUBSTR(PI_DATA_REF,7,4),PI_DATA_REF);	
				  COMMIT;
			...
			OPEN c_apr_regiao;
             ...
             BEGIN			 
                              insert into GRZ_VALORES_REGIOES_APR_ANUAL (COD_UNIDADE,VLR_VENDA_ORC,VLR_VENDA_REAL,VLR_MARGEM_ORC,VLR_MARGEM_REAL,VLR_VENDA_LIQUIDA,VLR_FALTA_INVENTARIO,
							              VLR_LUCRO,VALE_TRANSP,HORAS_TRAB,REGIAO,FOLHA,COD_REDE,ANO)
                              VALUES (r_totais_orcamento.cod_unidade,r_totais_orcamento.VLR_VENDA_ORC,r_totais_orcamento.VLR_VENDA_REAL,r_totais_orcamento.VLR_MARGEM_ORC,r_totais_orcamento.VLR_MARGEM_REAL,
									  ...
									  r_totais_orcamento.CUSTO_FOLHA,PI_CODREDE,SUBSTR(PI_DATA_REF,7,4));
							COMMIT;		  
             END;							  
             END IF;
	   	     CLOSE c_apr_regiao;	 

CREATE OR REPLACE PROCEDURE GRZ_ORCAMENTO_APR_ANUAL_CIA_SP
...
DECLARE
	...
	Wi			    NUMBER;
	Wf			    NUMBER;
	wPerLucro		NUMBER(18,2);
	wLucroRede      NUMBER(18,2);
	wVendaLiqRede   NUMBER(18,2);
	wParamLucro     NUMBER(18,2);
	wUnidads   number;
    ...
    CURSOR c_unidades is
       select b.cod_grupo
             ,b.cod_quebra
         	 ,b.cod_unidade
             ,c.des_nome des_unidade
         from ge_grupos_unidades b
             ,ge_unidades c
        where b.cod_unidade = c.cod_unidade
          and b.cod_emp = pi_codemp
          and b.cod_grupo = PI_CODGRUPO
          and b.cod_unidade >= 0
          and b.cod_unidade <= 9999
		 and exists (select 1 from ge_grupos_unidades t
                       where t.cod_unidade = b.cod_unidade
                         and t.cod_grupo in (1912,1932,1942,1952,1972)
                         and t.cod_emp = 1)
        order by 1,2,3,4;
	...
	CURSOR c_totais_orcamento IS
              select sum(decode(a.des_chave,'01#01001',nvl(a.vlr_orcado,0)
                ...
                from or_valores a
                    ,grz_lojas_gzt_apr b
               where a.cod_unidade     = b.cod_unidade_de
                 and b.cod_unidade_para = r_unidades.cod_unidade
	...
	BEGIN
	 ...
	 select sum(Vlr_Lucro),sum(Vlr_Venda_Liquida) INTO wLucroRede,wVendaLiqRede  from (select    a.cod_unidade,
                ...
                from or_valores a
                    ...
                    ,grz_lojas_gzt_apr b
               where a.cod_emp = 1
				 and a.cod_unidade     = b.cod_unidade_de
                 and a.cod_unidade    = p.cod_pessoa
				 and p.ind_inativo    = 0
                 and a.cod_emp        = pi_codemp
                 and a.cod_orcamento  = PI_ORCAMENTO
                 and a.dta_orcamento >= '01/01/'||SUBSTR(PI_DATA_REF,7,4)
		   ...
		   wParamLucro :=  Round((nvl(wLucroRede,0) * 100)/ nvl(wVendaLiqRede,0),2);
	   ...
	   BEGIN
		...
		BEGIN
	       ...
	       begin
			...
			BEGIN
			select sum(decode(a.des_chave,'01#01001',nvl(a.vlr_realizado,0)
                                           ,'01#01003',nvl(a.vlr_realizado,0)
                                           ,'01#01005',nvl(a.vlr_realizado,0),0)) Vlr_Venda_Real
                  ...
                  where a.cod_emp = 1
				   ...
				   and a.dta_orcamento not in ('01/03/2019','01/04/2019','01/07/2019')
                   and (a.des_chave = '01#01001'
                   or a.des_chave = '01#01003'
				   or a.des_chave = '01#01005');
	         EXCEPTION
			 ...
			 begin
                    select sum(nvl(o.vlr_operacao,0)) vlr_operacao
                     ...
                     where t.num_seq     = o.num_seq
                       ...
                       and trunc(t.dta_emissao) between '01/01/'||SUBSTR(add_months(PI_DATA_REF,-12),7,4) and '31/'||SUBSTR(add_months(PI_DATA_REF,-12),4,7)
			           and (t.dta_emissao not between '01/03/2019' and '30/04/2019'
                       and t.dta_emissao not between '01/07/2019' and '31/07/2019')
                       and t.cod_unidade     = b.cod_unidade_de
                       and b.cod_unidade_para = r_unidades.cod_unidade
			 ...
			 begin
                    select sum(nvl(o.vlr_operacao,0)) vlr_operacao
                     ...
                     where t.num_seq     = o.num_seq
			           ...
			           and (t.dta_emissao not between '01/03/2020' and '30/04/2020'
                       and t.dta_emissao not between '01/07/2020' and '31/07/2020')
                       and t.cod_unidade     = b.cod_unidade_de
                       and b.cod_unidade_para = r_unidades.cod_unidade
                       and nvl(t.ind_status,0) <> 2
                       and o.cod_oper in (106, 107);
			 ...
			 wVlr_Venda_Bruta_Sem_Devol_Ant := wVlr_Venda_Bruta_Ano_Ant - wValor_Devolucao_ano_ant;

			 if wVlr_Venda_Bruta_Sem_Devol_Ant > 0 then
			    wCrescimento := round(((wVlr_Venda_Bruta_Sem_Devol * 100) / wVlr_Venda_Bruta_Sem_Devol_Ant) - 100,2);
			 else
			    wCrescimento := 0;
			 end if;
			...
			if r_totais_orcamento.vlr_venda_liquida > 0 then
			   ...
			   wMargem_Percentual := 99;
		        ...
		        if PI_CODREDE = 10 then
                           ...
                           if wMargem_Percentual >= 56.0 then
                              wPontosMargemPerc := 10;
                           else
                              wPontosMargemPerc := 0;
                           end if;
		...
		---CUSTO DE FOLHA -----
			...
			wRegiao := 0;
			   ...
			   where cod_unidade = r_unidades.cod_unidade and cod_grupo in (211739,211740,211741);
            EXCEPTION
            WHEN NO_DATA_FOUND THEN
               wRegiao := 0;
			END;
		 ...
		 */
             ...
             ELSE
             BEGIN
                              insert into GRZ_DADOS_CALCULO_APR_ANUAL (COD_UNIDADE,ORCADO,MARGEM,LUCRO,INVENTARIO,VALE_TRANSP,PRODUTIVIDADE,FOLHA,COD_REDE,TIP_UNIDADE,REGIAO,ANO,DATA_REFERENCIA,MARGEM_PERC,CRESCIMENTO)
                              VALUES (r_unidades.cod_unidade,wPontosOrcado,wPontosMargem,wQtd_Pontos_Lucro,wQtd_Pontos_Inventario,wQtd_Pontos_ValeT,wQtd_Pontos_produt,wQtd_Pontos_Folha,
							  PI_CODREDE,1,wRegiao,SUBSTR(PI_DATA_REF,7,4),PI_DATA_REF,wPontosMargemPerc,wPontosCresc);
				  COMMIT;
			...
			OPEN c_apr_regiao;
             ...
             BEGIN
                              insert into GRZ_VALORES_REGIOES_APR_ANUAL (COD_UNIDADE,VLR_VENDA_ORC,VLR_VENDA_REAL,VLR_MARGEM_ORC,VLR_MARGEM_REAL,VLR_VENDA_LIQUIDA,VLR_FALTA_INVENTARIO,
							              VLR_LUCRO,VALE_TRANSP,HORAS_TRAB,REGIAO,FOLHA,COD_REDE,ANO,VLR_DEVOLUCAO,MARGEM_PERC,VLR_VENDA_ANO_ANT)
                              VALUES (r_unidades.cod_unidade,r_totais_orcamento.VLR_VENDA_ORC,r_totais_orcamento.VLR_VENDA_REAL,r_totais_orcamento.VLR_MARGEM_ORC,r_totais_orcamento.VLR_MARGEM_REAL,
									  ...
									  r_totais_orcamento.CUSTO_FOLHA,PI_CODREDE,SUBSTR(PI_DATA_REF,7,4),wValor_Devolucao,wMargem_Percentual,wVlr_Venda_Bruta_Sem_Devol_Ant);
							COMMIT;
             END;
             END IF;
	   	     CLOSE c_apr_regiao;
...
END;
END GRZ_ORCAMENTO_APR_ANUAL_CIA_SP;

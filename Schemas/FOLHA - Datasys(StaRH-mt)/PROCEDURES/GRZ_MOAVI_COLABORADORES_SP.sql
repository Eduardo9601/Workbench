...
BEGIN
  DECLARE
    ...
    CURSOR c_moavi IS
    SELECT DISTINCT A.NRO_PIS_PASEP AS PIS,
                    A.COD_CONTRATO AS MATRICULA,
                    A.DES_PESSOA AS NOME,
                    A.COD_UNIDADE AS FILIAL_COD,
  ...
  BEGIN
    cabecalho := 'pis;matricula;nome;filial_cod;filial_cnpj;status;cpf;cargo;ini_ferias;fim_ferias;genero';
      ...
      BEGIN
        ...
        ELSE
          UTL_FILE.PUT_LINE(file_handle1,
                            ...
                            wFILIAL_COD || ';' || wFILIAL_CNPJ || ';' ||
                            wSTATUS || ';' || wCPF || ';' || wCARGO || ';' ||
                            wINI_FERIAS || ';' || wFIM_FERIAS || ';' || wGENERO);
        END IF;
      END;
      FETCH c_moavi
        INTO r_moavi;

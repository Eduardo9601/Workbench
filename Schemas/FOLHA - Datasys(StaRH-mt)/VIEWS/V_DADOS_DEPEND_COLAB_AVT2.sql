...
SELECT --DADOS DEPENDENTE
       ...
       L.NOME_RESUMIDO AS DES_INSTRUCAO,
       C.DATA_OBITO,
       C.DATA_INICIO,
       C.DATA_FIM,
 ...
 WHERE A.COD_PESSOA = C.COD_FUNC
   ...
   AND C.COD_ESTADO_CIVIL = I.COD_ESTADO_CIVIL(+)
   AND C.TIPO_DEPENDENTE = J.COD_TIPO_DEPENDENTE(+)
   AND C.COD_NACIONALIDADE = K.COD_NACIONALIDADE(+)
   AND C.COD_GRAU_INSTRUCAO = L.COD_GRAU_INSTRUCAO(+)

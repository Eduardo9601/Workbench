...
REGIONAL AS (
  SELECT *
    FROM (SELECT DR.*,
                 ROW_NUMBER() OVER(PARTITION BY DR.REGIAO ORDER BY DR.COD_CONTRATO DESC) RN
...
SELECT UNI.COD_ATIVIDADE,
       ...
       UNI.COD_REGIAO,
       UNI.DES_REGIAO,
       DR.COD_CONTRATO AS COD_REGIONAL,
       DR.DES_REGIONAL,
  ...
  LEFT JOIN CONTRATOS CT ON UNI.COD_UNIDADE = CT.COD_UNIDADE
                        AND CT.COD_EMP = 8
                        AND CT.COD_TIPO = 1
  LEFT JOIN REGIONAL DR ON UNI.COD_REGIAO = DR.REGIAO

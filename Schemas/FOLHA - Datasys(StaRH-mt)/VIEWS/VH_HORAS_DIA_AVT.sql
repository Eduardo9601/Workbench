...
WITH DIA AS (
  SELECT A.COD_CONTRATO,
         ...
         B.COD_UNIDADE,
         B.DES_UNIDADE,
         TRUNC(A.DATA_OCORRENCIA) AS DATA_DIA, --formato de data é DD/MM/YYYY ex: 01/02/2026

         /* Dia da semana (texto) e n�mero ISO (segunda=1) */
         ...
         SUM(CASE
           ...
           /*uma possivel op��o*/  
         /*SUM(CASE
               -- S� soma a falta se a ocorr�ncia for falta E o dia j� tiver passado
...
SELECT COD_CONTRATO,
       ...
       /* formata��es HH:MM (mesma l�gica que voc� j� usa) */
       HORAS AS NUM_HORAS,
       CASE
         WHEN HORAS < 0 THEN
          '-' || TRUNC(ABS(HORAS)) || ':' ||

/*KPIs ANALYTCS - AJUSTANDO E VALIDANDO NOVA VERSÃO*/
...
DATAS_REF AS (
  ...
  /* MÊS I INICIANDO NO 1º DIA DO MÊS DE D_INI */
   ...
   TO_CHAR(P.D_INI, 'MM/YYYY') || ' - ' || TO_CHAR(P.D_FIM, 'MM/YYYY') AS MES_ANO_PER,
   ...
   INITCAP(TRIM(TO_CHAR(P.D_INI, 'Month'))) || '/' || TO_CHAR(P.D_INI, 'YYYY') || ' - ' ||
   INITCAP(TRIM(TO_CHAR(P.D_FIM, 'Month'))) || '/' || TO_CHAR(P.D_FIM, 'YYYY') AS MES_ANO_EXTENSO
    FROM PARAMS P
...
-- sem barrar TEMPORÁRIO!
 ...
 GROUP BY CTR.COD_ORGANOGRAMA, CTR.COD_UNIDADE, M.DTA_INI, CTR.COD_TIPO
),
...
-- sem barrar TEMPORÁRIO!
 ...
 GROUP BY CTR.COD_ORGANOGRAMA, CTR.COD_UNIDADE, M.DTA_FIM, CTR.COD_TIPO
),
...
CTE_HORAS AS (
    SELECT CT.COD_ORGANOGRAMA,
     ...
     CROSS JOIN DATAS_REF M
     WHERE HM.DATA_OCORRENCIA BETWEEN M.DTA_INI AND M.DTA_FIM
       AND (TU.DATA_FIM = '31/12/2999' OR TU.DATA_FIM >= M.DTA_FIM)
       AND (TU.DATA_INICIO <= M.DTA_FIM)
       AND TU.COD_TURNO <> 85
...
CTE_ATESTADOS_FALTAS AS (
    SELECT B.COD_ORGANOGRAMA,
     ...
     CROSS JOIN DATAS_REF M
     WHERE D.DATA_REFERENCIA BETWEEN M.DTA_INI AND M.DTA_FIM
...
HORAS_EXTRAS AS (
  SELECT CT.COD_EMP,
   ...
   WHERE A.COD_VD IN
     ...
     AND B.DATA_REFERENCIA BETWEEN M.DTA_INI AND M.DTA_FIM
   GROUP BY CT.COD_EMP,
            CT.COD_UNIDADE,
            CT.DES_UNIDADE,
            CT.COD_TIPO,
...
IRREGULARIDADES_PONTO AS (
  SELECT CT.COD_UNIDADE, CT.DES_UNIDADE, M.MES_ANO, CT.COD_CONTRATO, CT.COD_TIPO
   ...
   CROSS JOIN DATAS_REF M
   WHERE A.DATA_OCORRENCIA BETWEEN M.DTA_INI AND M.DTA_INI
     AND M.DTA_INI BETWEEN E.DATA_INICIO AND E.DATA_FIM
     AND (
         -- REGRA: +2HRS
...
PER_FEM_MASC AS (
  SELECT A.COD_UNIDADE,
    ...
    FROM CONTRATOS A
         ...
         CROSS JOIN (
           SELECT MAX(DTA_FIM) AS REF_DATA FROM DATAS_REF
         ) M
...
VENCIMENTOS AS (
  ...
  JOIN RHFP1003 R ON A.COD_MESTRE_EVENTO = R.COD_MESTRE_EVENTO
     AND R.DATA_REFERENCIA BETWEEN M.DTA_INI AND M.DTA_FIM
  ...
  WHERE B.TIPO_VD = 'V'
    ...
    AND (CT.COD_UNIDADE = :UNIDADE OR :UNIDADE = 0)
    AND (CT.COD_REDE = :REDE OR :REDE = 0)
  GROUP BY CT.COD_UNIDADE, M.DTA_INI, CT.COD_TIPO
),
ENCARGOS AS (
  SELECT
    CT.COD_UNIDADE,
...
BASE_CONTRATOS AS (
  SELECT CT.COD_UNIDADE,
         CT.COD_TIPO,
         COUNT(DISTINCT CT.COD_CONTRATO) AS ATIVOS_ATUAIS,

         COUNT(DISTINCT CASE
               WHEN CT.DATA_ADMISSAO <= M.DTA_FIM AND
                   (CT.DATA_DEMISSAO IS NULL OR CT.DATA_DEMISSAO >= M.DTA_FIM) THEN
                CT.COD_CONTRATO
             END) AS ATIVOS_PERIODO,

         COUNT(DISTINCT CASE
               WHEN CT.DATA_ADMISSAO BETWEEN M.DTA_INI AND M.DTA_FIM THEN
                CT.COD_CONTRATO
             END) AS ADMITIDOS,


         COUNT(DISTINCT CASE
                WHEN CT.DATA_DEMISSAO BETWEEN M.DTA_INI AND M.DTA_FIM THEN
                 CT.COD_CONTRATO
              END) AS DEMITIDOS,
         ...
         -- Faixas Etárias
         COUNT(DISTINCT CASE
                 WHEN FLOOR(MONTHS_BETWEEN(M.DTA_FIM, CT.DATA_NASCIMENTO) / 12) <= 30 THEN
                  CT.COD_CONTRATO
               END) AS QTDE_ATE_30,

         COUNT(DISTINCT CASE
                 WHEN FLOOR(MONTHS_BETWEEN(M.DTA_FIM, CT.DATA_NASCIMENTO) / 12) > 30 AND
                      FLOOR(MONTHS_BETWEEN(M.DTA_FIM, CT.DATA_NASCIMENTO) / 12) < 50 THEN
                  CT.COD_CONTRATO
               END) AS QTDE_30_50,

         COUNT(DISTINCT CASE
                 WHEN FLOOR(MONTHS_BETWEEN(M.DTA_FIM, CT.DATA_NASCIMENTO) / 12) >= 50 THEN
                  CT.COD_CONTRATO
               END) AS QTDE_50_MAIS,

         -- TEMPO MÉDIO DE EMPRESA FORMATADO
         FLOOR(AVG(MONTHS_BETWEEN(M.DTA_FIM, CT.DATA_ADMISSAO)) / 12) ||
         ' anos e ' ||
         MOD(FLOOR(AVG(MONTHS_BETWEEN(M.DTA_FIM, CT.DATA_ADMISSAO))), 12) ||
         ' meses' AS TEMPO_MEDIO_EMPRESA

    FROM CONTRATOS CT
    LEFT JOIN STATUS_AFASTADOS S ON CT.COD_CONTRATO = S.COD_CONTRATO
   CROSS JOIN DATAS_REF M
   WHERE /*M.DTA_FIM BETWEEN CT.DATA_ADMISSAO AND
...
SELECT --A.COD_UNIDADE AS COD_UNIDADE,
       A.DES_UNIDADE,
       A.DES_REDE,
       'Ref: ' || M.DTA_INI || ' - ' || M.DTA_FIM AS REFERENCIA,

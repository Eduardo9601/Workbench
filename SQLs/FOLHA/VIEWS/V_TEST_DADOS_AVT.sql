CREATE OR REPLACE VIEW V_TEST_DADOS_AVT AS
WITH CTE_ORGANOGRAMA AS (
      SELECT ORG.*,
        ...
        LEFT JOIN V_ORGANOGRAMAS_AVT VO ON ORG.COD_ORGANOGRAMA =
                                           VO.COD_ORGANOGRAMA
...
SELECT
    /*DADOS DO CONTRATO*/
     ...
     CT.COD_CONTRATO,
     CASE
       WHEN CT.DATA_INICIO > SYSDATE THEN
        NULL
       ELSE
     ...
     INNER JOIN RHFP0340 CT_FUNC ON CT.COD_CONTRATO = CT_FUNC.COD_CONTRATO
                                AND SYSDATE BETWEEN CT_FUNC.DATA_INICIO AND
                                    NVL(CT_FUNC.DATA_FIM,
                                        TO_DATE('31/12/2999', 'DD/MM/YYYY'))
      LEFT JOIN RHFP0500 FUNC ON CT_FUNC.COD_CLH = FUNC.COD_CLH
      LEFT JOIN V_PROGRAMACAO_FERIAS_AVT FER ON CT.COD_CONTRATO = FER.COD_CONTRATO AND FER.IND_FERIAS = 0
    -- DADOS DO ORGANOGRAMA DO CONTRATO
     INNER JOIN CTE_ORGANOGRAMA ORG ON CT.COD_CONTRATO = ORG.COD_CONTRATO
                                   AND ORG.RN = 1
    ...
    -- DADOS ANTERIORES DO CONTRATO
      LEFT JOIN CTE_CARGO_ANTERIOR CC ON CT.COD_CONTRATO = CC.COD_CONTRATO
                                     AND CC.RN = 1
      LEFT JOIN CTE_ORGANOGRAMA_ANTERIOR CO ON CT.COD_CONTRATO =
                                               CO.COD_CONTRATO
                                           AND CO.RN = 1
    ...
    -- DADOS DO TURNO E HORÁRIOS DO CONTRATO
     ...
     INNER JOIN V_HORAS_COLAB_AVT HR ON CT.COD_CONTRATO = HR.COD_CONTRATO
      LEFT JOIN RHFP0309 LOT_CONT ON CT.COD_CONTRATO = LOT_CONT.COD_CONTRATO AND SYSDATE BETWEEN LOT_CONT.DATA_INICIO
                                                                             AND NVL(LOT_CONT.DATA_FIM, TO_DATE('31/12/2999', 'DD/MM/YYYY'))
      LEFT JOIN RHFP0509 LOT ON LOT_CONT.COD_LOTACAO = LOT.COD_LOTACAO
      LEFT JOIN V_SINDICATOS_CONTRATOS_AVT SIND ON CT.COD_CONTRATO = SIND.COD_CONTRATO

     WHERE (
    (
        CT.DATA_FIM IS NULL AND
        SYSDATE BETWEEN CT.DATA_INICIO AND NVL(CT.DATA_FIM, TO_DATE('31/12/2999', 'DD/MM/YYYY'))
    )
    OR
    (
        CT.DATA_FIM > TRUNC(SYSDATE) + (1 - (1 / 86400))
        OR
        (
            CT.DATA_FIM = TRUNC(SYSDATE) + (1 - (1 / 86400))
...
/*SELECT A.COD_CONTRATO,
       A.DATA_INICIO AS DT_ADMISSAO,
       A.DATA_FIM AS DT_DEMISSAO,
       B.COD_CLH,
       B.DATA_INICIO AS DT_INI_FUNCAO,
       B.DATA_FIM AS DT_FIM_FUNCAO
FROM RHFP0300 A
INNER JOIN RHFP0340 B ON A.COD_CONTRATO = B.COD_CONTRATO

...
--OBS:Scripts corretos, sempre executar esses scripts, caso a datasys não tenha entregado uma solução via sistema
...
SELECT 1 AS COD_EVENTO,
       ...
       CC.DATA_REFERENCIA,
       'Aumento Salarial (Evento 22) - Mestre 16615' AS OBSERVACAO,
       NULL
FROM RHFP1005 CC
WHERE CC.COD_EVENTO = 22
  AND CC.DATA_REFERENCIA = TO_DATE('29/02/2024', 'DD/MM/YYYY')
  AND CC.COD_VD = 1090
  --AND CC.COD_CONTRATO = 385752
  AND CC.COD_CONTRATO NOT IN (SELECT FF.COD_CONTRATO
    ...
    AND CC.COD_CONTRATO IN (SELECT ORG.COD_CONTRATO
                            ...
                            WHERE CC.COD_CONTRATO = ORG.COD_CONTRATO 
						                --AND ORG.COD_CONTRATO = 385752 -- TESTANDO APENAS UM CONTRATO
                            AND ORG.DATA_FIM = '31/12/2999'
                            AND ORG.COD_NIVEL2 = 1629)/*INSERIR SOMENTE A EMPRESA 8 (DEMAIS EMPRESAS: 4, 276, 282, 1629)*/);

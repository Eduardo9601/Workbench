SELECT CT.COD_CONTRATO || ' - ' || PF.NOME_PESSOA AS COLABORADOR,
       ...
       SUBSTR(LPAD(PF.CPF, 11, '0'), 10, 2) AS CPF,
       ...
       TO_CHAR(SUM(VLR5.VALOR_VD), 'FM999G999G990D00') AS VALOR,
       ...
       UPPER(TO_CHAR(EV5.DATA_REFERENCIA, 'MONTH')) || '/' ||
       TO_CHAR (EV5.DATA_REFERENCIA, 'YYYY') AS REFERENCIA
...
LEFT JOIN RHFP1006 VLR5 ON CT.COD_CONTRATO = VLR5.COD_CONTRATO
...
WHERE EV5.DATA_REFERENCIA BETWEEN :DATA_INICIO AND :DATA_FIM
  --AND CT.DATA_FIM IS NOT NULL
  AND EV5.COD_EVENTO = 8
  AND VLR5.COD_VD = 1003
  --AND VLR6.VALOR_VD <> 0
  AND EV5.SITUACAO_PROCESSO = 'I'

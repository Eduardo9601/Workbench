/*VERSÃO OFICIAL - COM DATAS FIXAS PRA EXECUTAR NO PL/SQL*/
...
IRREGULARIDADES_PONTO AS (
  SELECT CT.COD_UNIDADE, CT.DES_UNIDADE, CT.COD_CONTRATO
    FROM RHAF1123 A
...
IRREGULARIDADES AS (
  SELECT COD_UNIDADE,
         DES_UNIDADE,
         COUNT(DISTINCT COD_CONTRATO) AS QTD_COLAB_IRREGULAR
...
SELECT A.COD_UNIDADE,
       ...
       NVL(B1.QTD_AFASTADOS, 0) AS AFASTADOS,
       NVL(B1.QTD_APRENDIZ, 0) AS APRENDIZ,
       NVL(B1.PERC_APRENDIZ, 0) AS "%_APREN",
       NVL(B1.QTD_PCD, 0) AS PCD,
       NVL(B1.PERC_PCD, 0) AS "%_PCD",
       NVL(B1.QTDE_ATE_30, 0) AS ATE_30_ANOS,
       NVL(B1.QTDE_30_50, 0) AS DE_30_50_ANOS,
       NVL(B1.QTDE_50_MAIS, 0) AS DE_50_ANOS_MAIS,
       NVL(B1.TEMPO_MEDIO_EMPRESA, 0) AS TEMPO_MEDIO_EMPRESA
...
-- KPIs principais
  ...
  LEFT JOIN BASE_CONTRATOS B1 ON A.COD_UNIDADE = B1.COD_UNIDADE
...
---com entrada de parâmetros OFICIAL ATÉ aqui
--esse é o original, com as datas fixas
...
CTE_HORAS AS (
    SELECT CT.COD_ORGANOGRAMA,
     ...
     WHERE HM.DATA_OCORRENCIA BETWEEN M.MONTH_START AND M.MONTH_END
       ...
       AND (CT.COD_UNIDADE = :FILIAL OR :FILIAL = 0)
...
/*AJUSTANDO A LINHA PRA GERAR OS TOTAIS*/
...
SELECT CASE
       ...
       MIN(D.DTA_CADASTRO) DTA_CADASTRO, -- IDEM
       MIN(A.DTA_EMISSAO) DTA_ATUALIZACAO,
       SUM(OPER.VLR_OPERACAO) VALOR_VENDA_LOJA_LJ,
       SUM(CASE
             WHEN TO_CHAR(A.DTA_EMISSAO, 'D') = 1 THEN
              NVL(OPER.VLR_OPERACAO, 0)
             ELSE
...
SELECT TT.COD_TIPO,
       TT.TOTAL_HRS_LJ,
       TRUNC(A.VALOR_VENDA_LOJA_LJ / NULLIF(TT.TOTAL_HRS_LJ, 0)) AS PRODUTIVIDADE_LJ
  FROM VENDAS_LJ A
  LEFT JOIN TOTAL_HORAS_LJ TT ON A.COD_EMP = TT.COD_EMP
 GROUP BY TT.COD_TIPO, A.VALOR_VENDA_LOJA_LJ, TT.TOTAL_HRS_LJ
),
...
EFETIVO_INICIAL_TUR_LJ AS (
    SELECT CTR.COD_TIPO,
           M.MONTH_START,
           COUNT(DISTINCT CTR.COD_CONTRATO) AS EFETIVO_INICIAL_LJ
      FROM CONTRATOS CTR
     CROSS JOIN MONTHS M
     WHERE ('01/' || TO_CHAR(M.MONTH_START, 'MM/YYYY')) BETWEEN
           CTR.DATA_ADMISSAO AND NVL(CTR.DATA_DEMISSAO, '31/12/9999')
       ...
       AND (UPPER(CTR.DES_FUNCAO) NOT LIKE '%ESTAGIARIO%' AND
           ...
           UPPER(CTR.DES_FUNCAO) NOT LIKE '%TEMPORARIO%')
       AND CTR.COD_TIPO = 1
     GROUP BY CTR.COD_TIPO, M.MONTH_START
),
EFETIVO_FINAL_TUR_LJ AS (
    SELECT CTR.COD_TIPO,
           M.MONTH_END,
           COUNT(DISTINCT CTR.COD_CONTRATO) AS EFETIVO_FINAL_LJ
      FROM CONTRATOS CTR
     CROSS JOIN MONTHS M
     WHERE LAST_DAY(TO_DATE('' || TO_CHAR(M.MONTH_END, 'DD/MM/YYYY'))) BETWEEN
           CTR.DATA_ADMISSAO AND NVL(CTR.DATA_DEMISSAO, '31/12/9999')
       ...
       AND (UPPER(CTR.DES_FUNCAO) NOT LIKE '%ESTAGIARIO%' AND
            ...
            UPPER(CTR.DES_FUNCAO) NOT LIKE '%TEMPORARIO%')
       AND CTR.COD_TIPO = 1
     GROUP BY CTR.COD_TIPO, M.MONTH_END
),
...
TURNOVER_LJ AS (
SELECT A.COD_TIPO,
       (EI.EFETIVO_INICIAL_LJ + EF.EFETIVO_FINAL_LJ) / 2 AS EFETIVO_MEDIO_LJ,
       ROUND(CASE
               WHEN (EI.EFETIVO_INICIAL_LJ + EF.EFETIVO_FINAL_LJ) / 2 = 0 THEN
                0
               ELSE
                (DM.DEMITIDOS_LJ * 100) /
                ((EI.EFETIVO_INICIAL_LJ + EF.EFETIVO_FINAL_LJ) / 2)
             END,
             2) AS TURNOVER_LJ
  FROM (SELECT DISTINCT COD_TIPO
          FROM CONTRATOS) A
  LEFT JOIN EFETIVO_INICIAL_TUR_LJ EI ON A.COD_TIPO = EI.COD_TIPO
  LEFT JOIN EFETIVO_FINAL_TUR_LJ EF ON A.COD_TIPO = EF.COD_TIPO
  LEFT JOIN DEMITIDOS_TUR_LJ DM ON A.COD_TIPO = DM.COD_TIPO
),
...
SELECT A.COD_TIPO,
       ...
       ROUND(CASE
               WHEN (EI.EFETIVO_INICIAL_LJ + EF.EFETIVO_FINAL_LJ) / 2 = 0 THEN
                0
               ELSE
                (((NVL(AD.ADMITIDOS_LJ, 0) + NVL(DM.DEMITIDOS_LJ, 0)) / 2) * 100) /
                ((EI.EFETIVO_INICIAL_LJ + EF.EFETIVO_FINAL_LJ) / 2)
             END,
             2) AS ROTATIVIDADE_LJ
  FROM (SELECT DISTINCT COD_TIPO
          FROM CONTRATOS) A
  LEFT JOIN EFETIVO_INICIAL_ROT_LJ EI ON A.COD_TIPO = EI.COD_TIPO
  LEFT JOIN EFETIVO_FINAL_ROT_LJ EF ON A.COD_TIPO = EF.COD_TIPO
  LEFT JOIN DEMITIDOS_ROT_LJ DM ON A.COD_TIPO = DM.COD_TIPO
  LEFT JOIN ADMITIDOS_ROT_LJ AD ON A.COD_TIPO = AD.COD_TIPO
...
CTE_HORAS_LJ AS (
    ...
    /* AND (B.COD_UNIDADE = :FILIAL OR :FILIAL = 0)
       AND (B.COD_REDE = TO_CHAR(:REDE) OR TO_CHAR(:REDE) = 0)*/
    --AND HM.COD_CONTRATO = 392184
     GROUP BY CT.COD_TIPO

),


CTE_ATESTADOS_FALTAS_LJ AS (
    SELECT B.COD_TIPO,
           -- TOTAL FALTAS
           SUM(CASE WHEN C.COD_VD = 632 THEN C.QTDE_VD ELSE 0 END) AS TOT_FALTAS_LJ,

           -- TOTAL ATESTADOS
           SUM(CASE WHEN C.COD_VD = 897 THEN C.QTDE_VD ELSE 0 END) AS TOT_ATESTADOS_LJ,

           -- TOTAL FALTAS + ATESTADOS
           SUM(CASE WHEN C.COD_VD IN (632, 897) THEN C.QTDE_VD ELSE 0 END) AS TOT_FALTAS_ATESTADOS_LJ
           --TO_HOUR_MINUTES_03(SUM(C.QTDE_VD)) AS TOTAL_HORAS_ATESTADOS
      FROM RHFP1006 C
     INNER JOIN RHFP1003 D ON C.COD_MESTRE_EVENTO = D.COD_MESTRE_EVENTO
     INNER JOIN CONTRATOS B ON C.COD_CONTRATO = B.COD_CONTRATO
...
ABSENTEISMO_LJ AS (
SELECT H.COD_TIPO,
       NVL(ROUND(NVL(FT.TOT_FALTAS_ATESTADOS_LJ, 0) * 100 / NULLIF(NVL(H.TOT_HORAS_TRAB_LJ, 0), 0), 2), 0) AS ABSENTEISMO_LJ
FROM CTE_HORAS_LJ H
 LEFT JOIN CTE_ATESTADOS_FALTAS_LJ FT ON H.COD_TIPO = FT.COD_TIPO
...
REPOUSO_HORAS_EXTRAS_LJ AS (
  SELECT CT.COD_TIPO,
         SUM(CASE
               ...
               ELSE
                0
             END) AS QTDE_REP_HORAS_EXTRAS_LJ,
         SUM(CASE
               WHEN A.COD_VD = 158 THEN
                COALESCE(A.VALOR_VD, 0)
               ELSE
                0
             END) AS VLR_REP_HORAS_EXTRAS_LJ,
         B.DATA_REFERENCIA,
         B.COD_EVENTO
    FROM RHFP1006 A
   INNER JOIN RHFP1003 B ON A.COD_MESTRE_EVENTO = B.COD_MESTRE_EVENTO
...
BONUS_DOMINGO_LJ AS (
  SELECT CT.COD_TIPO,
         SUM(CASE
               ...
               ELSE
                0
             END) AS QTDE_BONUS_DOM_LJ,
         SUM(CASE
               WHEN A.COD_VD = 323 THEN
                COALESCE(A.VALOR_VD, 0)
               ELSE
                0
             END) AS VLR_BONUS_DOM_LJ,
         B.DATA_REFERENCIA,
         B.COD_EVENTO
    FROM RHFP1006 A
   INNER JOIN RHFP1003 B ON A.COD_MESTRE_EVENTO = B.COD_MESTRE_EVENTO
...
HRS_EXTRAS_LJ AS (
SELECT CT.COD_TIPO,
       NVL(HE.QTDE_HORAS_EXTRAS_LJ, 0) AS QTDE_HORAS_EXTRAS_LJ,
       NVL(HE.VLR_HORAS_EXTRAS_LJ, 0) AS VLR_HORAS_EXTRAS_LJ,
       NVL(RHE.VLR_REP_HORAS_EXTRAS_LJ, 0) AS VLR_REPOUSO_HR_EXTRAS_LJ,
       NVL(BD.VLR_BONUS_DOM_LJ, 0) AS VLR_BONUS_DOM_LJ
  FROM CONTRATOS CT
  LEFT JOIN HORAS_EXTRAS_LJ HE ON CT.COD_TIPO = HE.COD_TIPO
  LEFT JOIN REPOUSO_HORAS_EXTRAS_LJ RHE ON CT.COD_TIPO = RHE.COD_TIPO
  LEFT JOIN BONUS_DOMINGO_LJ BD ON CT.COD_TIPO = BD.COD_TIPO
...
IRREGULARIDADES_LJ AS (
  SELECT COD_TIPO,
         COUNT(DISTINCT COD_CONTRATO) AS QTD_COLAB_IRREGULAR_LJ
    FROM IRREGULARIDADES_PONTO_LJ
   GROUP BY COD_TIPO
),
...
PER_FEM_MASC_LJ AS (
  SELECT A.COD_TIPO,
         COUNT(DISTINCT CASE WHEN A.SEXO = 'F' THEN A.COD_CONTRATO END) AS QTD_COLAB_FEM_LJ,
         ROUND(COUNT(CASE WHEN A.SEXO = 'F'  THEN 1 END) * 100.0 / COUNT(*), 2) AS PERC_FEM_LJ,
         COUNT(DISTINCT CASE WHEN A.SEXO = 'M' THEN A.COD_CONTRATO END) AS QTD_COLAB_MASC_LJ,
         ROUND(COUNT(CASE WHEN A.SEXO = 'M' THEN 1 END) * 100.0 / COUNT(*), 2) AS PERC_MASC_LJ,
         COUNT(DISTINCT A.COD_CONTRATO) AS TOT_COLAB_LJ
    FROM CONTRATOS A
         LEFT JOIN STATUS_AFASTADOS S ON A.COD_CONTRATO = S.COD_CONTRATO
         CROSS JOIN (
           SELECT MAX(MONTH_END) AS REF_DATA FROM MONTHS
...
VENCIMENTOS_LJ AS (
  ...
  WHERE B.TIPO_VD = 'V'
    AND A.VALOR_VD <> 0
  GROUP BY CT.COD_TIPO, M.MONTH_START
),
ENCARGOS_LJ AS (
  SELECT
    CT.COD_TIPO,
...
CUSTO_FOLHA_LJ AS (
  SELECT
    V.COD_TIPO,
    V.MONTH_START,
    NVL(V.TOTAL_VENCIMENTOS,0) AS TOTAL_VENCIMENTOS_LJ,
    NVL(E.TOTAL_ENCARGOS,0) AS TOTAL_ENCARGOS_LJ,
    (V.TOTAL_VENCIMENTOS + NVL(E.TOTAL_ENCARGOS,0)) AS CUSTO_FOLHA_LJ
  FROM VENCIMENTOS_LJ V
  LEFT JOIN ENCARGOS_LJ E
    ON V.COD_TIPO = E.COD_TIPO
   AND V.MONTH_START = E.MONTH_START
...
BASE_CONTRATOS_LJ AS (
  SELECT CT.COD_TIPO,
         ...
         COUNT(DISTINCT CASE
                 WHEN S.STATUS_AFAST NOT IN (0, 6, 7, 107) THEN
                  CT.COD_CONTRATO
               END) AS QTD_AFASTADOS_LJ,

         -- APRENDIZ
         COUNT(DISTINCT CASE
                 WHEN UPPER(CT.DES_FUNCAO) LIKE '%APRENDIZ%' THEN
                  CT.COD_CONTRATO
               END) AS QTD_APRENDIZ_LJ,
         ...
         COUNT(DISTINCT CASE
                 WHEN FLOOR(MONTHS_BETWEEN(M.MONTH_END, CT.DATA_NASCIMENTO) / 12) <= 30 THEN
                  CT.COD_CONTRATO
               END) AS QTDE_ATE_30_LJ,

         COUNT(DISTINCT CASE
                 WHEN FLOOR(MONTHS_BETWEEN(M.MONTH_END, CT.DATA_NASCIMENTO) / 12) > 30 AND
                      FLOOR(MONTHS_BETWEEN(M.MONTH_END, CT.DATA_NASCIMENTO) / 12) < 50 THEN
                  CT.COD_CONTRATO
               END) AS QTDE_30_50_LJ,

         COUNT(DISTINCT CASE
                 WHEN FLOOR(MONTHS_BETWEEN(M.MONTH_END, CT.DATA_NASCIMENTO) / 12) >= 50 THEN
                  CT.COD_CONTRATO
               END) AS QTDE_50_MAIS_LJ,

         -- TEMPO MÉDIO DE EMPRESA FORMATADO
         FLOOR(AVG(MONTHS_BETWEEN(M.MONTH_END, CT.DATA_ADMISSAO)) / 12) ||
         ' anos e ' ||
         MOD(FLOOR(AVG(MONTHS_BETWEEN(M.MONTH_END, CT.DATA_ADMISSAO))), 12) ||
         ' meses' AS TEMPO_MEDIO_EMPRESA_LJ
...
-- Versão corrigida do SELECT final para TOTAL GERAL consolidado por COD_TIPO
SELECT
       A.COD_TIPO,
       'TOTAL GERAL' AS DES_REDE,

       SUM(PFM.TOT_COLAB_LJ) AS COLABORADORES,

       ROUND(AVG(B.TURNOVER_LJ), 2) AS "%_TURNOVER",
       ROUND(SUM(C.PRODUTIVIDADE_LJ), 2) AS PRODUTIVIDADE,
       ROUND(AVG(E.ABSENTEISMO_LJ), 2) AS "%_ABSENTEISMO",
       ROUND(AVG(D.ROTATIVIDADE_LJ), 2) AS "%_ROTATIVIDADE",
       SUM(G.QTD_COLAB_IRREGULAR_LJ) AS QTD_IRREGULARIDADES_PONTO,

       ROUND(SUM(F.QTDE_HORAS_EXTRAS_LJ), 2) AS QTD_HORAS_EXTRAS,
       TO_CHAR(SUM(F.VLR_HORAS_EXTRAS_LJ), 'FM999G999G990D00') AS VLR_HORAS_EXTRAS,
       TO_CHAR(SUM(F.VLR_REPOUSO_HR_EXTRAS_LJ), 'FM999G999G990D00') AS VLR_REPOUSO_HR_EXTRAS,
       TO_CHAR(SUM(F.VLR_BONUS_DOM_LJ), 'FM999G999G990D00') AS VLR_BONUS_DOM,

       TO_CHAR(SUM(CF.TOTAL_VENCIMENTOS_LJ), 'FM999G999G990D00') AS VENCIMENTOS,
       TO_CHAR(SUM(CF.TOTAL_ENCARGOS_LJ), 'FM999G999G990D00') AS ENCARGOS,
       TO_CHAR(SUM(CF.CUSTO_FOLHA_LJ), 'FM999G999G990D00') AS CUSTO_FOLHA,

       SUM(PFM.QTD_COLAB_FEM_LJ) AS FEMININO,
       ROUND(SUM(PFM.QTD_COLAB_FEM_LJ) * 100.0 / NULLIF(SUM(PFM.TOT_COLAB_LJ), 0), 2) AS "%_FEM",
       SUM(PFM.QTD_COLAB_MASC_LJ) AS MASCULINO,
       ROUND(SUM(PFM.QTD_COLAB_MASC_LJ) * 100.0 / NULLIF(SUM(PFM.TOT_COLAB_LJ), 0), 2) AS "%_MASC",

       SUM(BC.QTD_AFASTADOS_LJ) AS AFASTADOS,
       SUM(BC.QTD_APRENDIZ_LJ) AS APRENDIZ,
       ROUND(SUM(BC.QTD_APRENDIZ_LJ) * 100.0 / NULLIF(SUM(PFM.TOT_COLAB_LJ), 0), 2) AS "%_APREN",
       SUM(BC.QTD_PCD_LJ) AS PCD,
       ROUND(SUM(BC.QTD_PCD_LJ) * 100.0 / NULLIF(SUM(PFM.TOT_COLAB_LJ), 0), 2) AS "%_PCD",

       SUM(BC.QTDE_ATE_30_LJ) AS ATE_30_ANOS,
       SUM(BC.QTDE_30_50_LJ) AS DE_30_50_ANOS,
       SUM(BC.QTDE_50_MAIS_LJ) AS DE_50_ANOS_MAIS,

       MAX(BC.TEMPO_MEDIO_EMPRESA_LJ) AS TEMPO_MEDIO_EMPRESA

  FROM (SELECT DISTINCT COD_TIPO FROM CONTRATOS) A

  JOIN TURNOVER_LJ B ON A.COD_TIPO = B.COD_TIPO
  LEFT JOIN PRODUTIVIDADE_LJ C ON A.COD_TIPO = C.COD_TIPO
  LEFT JOIN ROTATIVIDADE_LJ D ON A.COD_TIPO = D.COD_TIPO
  LEFT JOIN ABSENTEISMO_LJ E ON A.COD_TIPO = E.COD_TIPO
  LEFT JOIN HRS_EXTRAS_LJ F ON A.COD_TIPO = F.COD_TIPO
  LEFT JOIN IRREGULARIDADES_LJ G ON A.COD_TIPO = G.COD_TIPO
  LEFT JOIN PER_FEM_MASC_LJ PFM ON A.COD_TIPO = PFM.COD_TIPO
  LEFT JOIN BASE_CONTRATOS_LJ BC ON A.COD_TIPO = BC.COD_TIPO
  LEFT JOIN CUSTO_FOLHA_LJ CF ON A.COD_TIPO = CF.COD_TIPO

 WHERE (B.TURNOVER_LJ IS NOT NULL OR B.TURNOVER_LJ <> 0)
   AND (C.PRODUTIVIDADE_LJ IS NOT NULL OR C.PRODUTIVIDADE_LJ <> 0)
   AND (D.ROTATIVIDADE_LJ IS NOT NULL OR D.ROTATIVIDADE_LJ <> 0)
   AND (E.ABSENTEISMO_LJ IS NOT NULL OR E.ABSENTEISMO_LJ <> 0)

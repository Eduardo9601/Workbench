...
SELECT TT.COD_TIPO,
 ...
 GROUP BY TT.COD_TIPO, A.VALOR_VENDA_LOJA, TT.TOTAL_HRS
),
...
SELECT A.COD_TIPO,
  ...
  FROM (SELECT DISTINCT COD_TIPO,
                        ...
                        COD_REDE
        FROM CONTRATOS) A
  LEFT JOIN EFETIVO_INICIAL_TUR EI ON A.COD_TIPO =
                                      EI.COD_TIPO
  LEFT JOIN EFETIVO_FINAL_TUR EF ON A.COD_TIPO = EF.COD_TIPO
  LEFT JOIN DEMITIDOS_TUR DM ON A.COD_TIPO = DM.COD_TIPO
  WHERE (A.COD_TIPO = :TIPO OR :TIPO = 0)
    AND (A.COD_UNIDADE = :UNIDADE OR :UNIDADE = 0)
    AND (A.COD_REDE = :REDE OR :REDE = 0)
...
SELECT A.COD_TIPO,
  ...
  FROM (SELECT DISTINCT COD_TIPO,
                        ...
                        COD_REDE
          FROM CONTRATOS) A
  LEFT JOIN EFETIVO_INICIAL_ROT EI ON A.COD_TIPO =
                                      EI.COD_TIPO
  LEFT JOIN EFETIVO_FINAL_ROT EF ON A.COD_TIPO = EF.COD_TIPO
  LEFT JOIN DEMITIDOS_ROT DM ON A.COD_TIPO = DM.COD_TIPO
  LEFT JOIN ADMITIDOS_ROT AD ON A.COD_TIPO = AD.COD_TIPO
  WHERE (A.COD_TIPO = :TIPO OR :TIPO = 0)
    AND (A.COD_UNIDADE = :UNIDADE OR :UNIDADE = 0)
    AND (A.COD_REDE = :REDE OR :REDE = 0)
...
SELECT H.COD_TIPO,
       ...
       NVL(ROUND(NVL(FT.TOT_FALTAS_ATESTADOS, 0) * 100 / NULLIF(NVL(H.TOT_HORAS_TRAB, 0), 0), 2), 0) AS ABSENTEISMO
FROM CTE_HORAS H
 LEFT JOIN CTE_ATESTADOS_FALTAS FT ON H.COD_TIPO = FT.COD_TIPO
...
SELECT CT.COD_TIPO,
       ...
       NVL(BD.VLR_BONUS_DOM, 0) AS VLR_BONUS_DOM
  FROM CONTRATOS CT
  LEFT JOIN HORAS_EXTRAS HE ON CT.COD_TIPO = HE.COD_TIPO
  LEFT JOIN REPOUSO_HORAS_EXTRAS RHE ON CT.COD_TIPO = RHE.COD_TIPO
  LEFT JOIN BONUS_DOMINGO BD ON CT.COD_TIPO = BD.COD_TIPO
  WHERE (CT.COD_TIPO = :TIPO OR :TIPO = 0)
    AND (CT.COD_UNIDADE = :UNIDADE OR :UNIDADE = 0)
    AND (CT.COD_REDE = :REDE OR :REDE = 0)
...
SELECT NULL AS COD_UNIDADE,
       NULL AS DES_UNIDADE,
       'TOTAL DOS KPIs' AS DES_REDE,
...
-- KPIs principais
  ...
  LEFT JOIN HRS_EXTRAS_LJ F ON A.COD_TIPO = F.COD_TIPO -- Corrigido para refletir unidade
  LEFT JOIN IRREGULARIDADES_LJ G ON A.COD_TIPO = G.COD_TIPO
  LEFT JOIN PER_FEM_MASC_LJ PFM ON PFM.COD_TIPO = A.COD_TIPO
  LEFT JOIN BASE_CONTRATOS_LJ B ON A.COD_TIPO = B.COD_TIPO

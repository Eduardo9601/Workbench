...
ORDER BY COD_UNIDADE, DTA_INI

drop table GRZ_KPI_AVALIACAO_PLR_TB

*/


/* ====================================================================================================
   ==== PROCEDIMENTO DE INSERÇÃO MANUAL DOS DADOS DE KPIs COMERCIAIS E DE RH PARA AVALIAÇÃO DA PLR ====
   ==================================================================================================== */


/*PASSO 1  - CRIAÇÃO DA TABELA QUE ARMAZENARÁ OS DADOS DOS KPIs do SQL OFICIAL*/
...
/*CREATE TABLE GRZ_KPI_AVALIACAO_PLR_TB (
  ...
  EFETIVO_INICIAL        NUMBER(18,2),
  EFETIVO_FINAL          NUMBER(18,2),
...
*/
/*PASSO 2 - INSERÇÃO DOS DADOS DO SQL KPIs NA TABELA FINAL*/
...
/* ==============================================================================
   ========= KPIS PARA ACOMPANHAMENTO DA AVALIAÇÃO DA APR - SQL OFICIAL ========= 
   ============================================================================== */
   
-- EM FASE DE AJUSTES E VALIDAÇÕES DE CONSCISTÊNCIA E PERSISTÊNCIA DE DADOS...
...
SELECT A.COD_UNIDADE,
 ...
 GROUP BY A.COD_UNIDADE, A.VALOR_VENDA_LOJA, A.VDA_DOMINGO, TT.TOTAL_HRS, TT.COD_TIPO
...
EFETIVO_INICIAL_TUR AS (
    SELECT CTR.COD_ORGANOGRAMA,
     ...
     WHERE ('01/' || TO_CHAR(P.D_INI, 'MM/YYYY')) BETWEEN
       ...
       AND ('01/' || TO_CHAR(P.D_INI, 'MM/YYYY')) BETWEEN
     ...
     GROUP BY CTR.COD_ORGANOGRAMA, CTR.COD_UNIDADE, P.D_INI, CTR.COD_TIPO
),
...
/* =========================================================================
   ==== PROCEDIMENTO PRA ATUALIZAR A COLUNA NOVA DE QUANTIDADE DE MESES ==== 
   ====    - SEMPRE CONTANDO 1 PARA CADA PERÍODO PRESENTE NA TABELA     ====
   ========================================================================= */
   
-- Preenche com o ÍNDICE do mês (1..N) para cada linha
...
USING (
  ...
  DATAS_REF AS (
    SELECT
      ...
      MONTHS_BETWEEN(ADD_MONTHS(TRUNC(P.D_FIM,'MM'),1), TRUNC(P.D_INI,'MM'))  AS QTD_MESES
    FROM PARAMS P
    CONNECT BY LEVEL <= MONTHS_BETWEEN(ADD_MONTHS(TRUNC(P.D_FIM,'MM'),1), TRUNC(P.D_INI,'MM'))
  )
  SELECT DTA_INI, DTA_FIM, IDX_MES, QTD_MESES FROM DATAS_REF
) D
ON ( TRUNC(T.DTA_INI) = TRUNC(D.DTA_INI)
     AND TRUNC(T.DTA_FIM) = TRUNC(D.DTA_FIM) )
WHEN MATCHED THEN
  UPDATE SET T.MESES_REF = D.IDX_MES;